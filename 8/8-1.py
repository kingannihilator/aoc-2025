...
for connection in range(len(boxes)):
...
'''
go through min distances one by one:
    if both are in a circuit:
        continue
    if r already in a circuit but c is not (vice versa):
        add c to that circuit
    if neither is in a circuit:
        make new circuit
